...
fn main() {
    ...
    let mut circuit = Circuit::new(num_qubits).with_name("Grover's Algorithm");
    ...
    let iterations =
        ((std::f64::consts::PI / 4.0) * (1 << num_qubits as u32) as f64).sqrt() as usize;
...
fn apply_oracle(circuit: &mut Circuit, target_state: usize, num_qubits: usize) {
    ...
    if num_qubits == 3 {
        circuit.h(2);
        circuit
            .add_operation(toffoli_gate(), vec![0, 1, 2], "CCZ")
            .unwrap();
        circuit.h(2);
    } else {
...
fn apply_multi_controlled_not(circuit: &mut Circuit, controls: &[usize], target: usize) {
    ...
    if controls.len() == 2 {
        ...
        println!(
            "Warning: Multi-controlled NOT with {} control qubits is being simulated directly",
            controls.len()
        );
