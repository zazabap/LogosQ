...
impl Visualizable for State {
    fn visualize(&self) -> String {
        ...
        let mut output = format!("Quantum State: {} qubit{}\n\n",
            self.num_qubits,
        ...
        for (i, amplitude) in self.vector.iter().enumerate() {
            ...
            if probability > 1e-10 {
                let basis = format!("{}{}{}", "|", format!("{:0width$b}", i, width = self.num_qubits), ">");
                output.push_str(&format!("{}: {:.4}{:+.4}i (p={:.4})\n",
    ...
    fn visualize_as_svg(&self) -> String {
        if self.num_qubits != 1 {
            return "3D Bloch sphere visualization is only available for single-qubit states.".to_string();
        }
    ...
    fn view(&self) -> Result<()> {
        ...
        #[cfg(target_os = "linux")]
        std::process::Command::new("xdg-open").arg(temp_file).spawn()?;

        #[cfg(target_os = "macos")]
        std::process::Command::new("open").arg(temp_file).spawn()?;

        #[cfg(target_os = "windows")]
        std::process::Command::new("cmd").args(&["/C", "start", temp_file]).spawn()?;
...
impl State {
    ...
    fn plot_bloch_sphere_3d(&self, filename: &str) -> Result<()> {
        ...
        if let Err(e) = root.fill(&WHITE) {
            return Err(Error::new(ErrorKind::Other, format!("Drawing error: {:?}", e)));
        }
        ...
        let mut chart = match ChartBuilder::on(&root)
            ...
            .build_cartesian_3d(-1.2..1.2, -1.2..1.2, -1.2..1.2) {
                Ok(chart) => chart,
                Err(e) => return Err(Error::new(ErrorKind::Other, format!("Chart error: {:?}", e)))
            };

        // Configure axes (view_angle is not available in plotters)
        if let Err(e) = chart.configure_axes()
        ...
        for i in 0..12 {
            ...
            let line_points: Vec<(f64, f64, f64)> = (0..=12).map(|j| {
            ...
            if let Err(e) = chart.draw_series(LineSeries::new(
                ...
                return Err(Error::new(ErrorKind::Other, format!("Longitude line error: {:?}", e)));
            }
        }
        ...
        for j in 1..12 {
            let theta = PI * (j as f64) / 12.0;
            let line_points: Vec<(f64, f64, f64)> = (0..=16).map(|i| {
            ...
            if let Err(e) = chart.draw_series(LineSeries::new(
                ...
                return Err(Error::new(ErrorKind::Other, format!("Latitude line error: {:?}", e)));
            }
        }
        ...
        if let Err(e) = chart.draw_series(LineSeries::new(
            ...
            return Err(Error::new(ErrorKind::Other, format!("X-axis drawing error: {:?}", e)));
        }
        ...
        if let Err(e) = chart.draw_series(LineSeries::new(
            ...
            return Err(Error::new(ErrorKind::Other, format!("Y-axis drawing error: {:?}", e)));
        }
        ...
        if let Err(e) = chart.draw_series(LineSeries::new(
            ...
            return Err(Error::new(ErrorKind::Other, format!("Z-axis drawing error: {:?}", e)));
        }
        ...
        if let Err(e) = chart.draw_series(PointSeries::of_element(
            ...
            BLACK,
            &|coord, _size, _style| {
                Text::new("X", coord, ("sans-serif", 20).into_font())
            },
        )) {
            return Err(Error::new(ErrorKind::Other, format!("X label error: {:?}", e)));
        }
        ...
        if let Err(e) = chart.draw_series(PointSeries::of_element(
            ...
            BLACK,
            &|coord, _size, _style| {
                Text::new("Y", coord, ("sans-serif", 20).into_font())
            },
        )) {
            return Err(Error::new(ErrorKind::Other, format!("Y label error: {:?}", e)));
        }
        ...
        if let Err(e) = chart.draw_series(PointSeries::of_element(
            ...
            BLACK,
            &|coord, _size, _style| {
                Text::new("Z", coord, ("sans-serif", 20).into_font())
            },
        )) {
            return Err(Error::new(ErrorKind::Other, format!("Z label error: {:?}", e)));
        }
        ...
        for (point, label) in reference_points.iter() {
            ...
            if let Err(e) = chart.draw_series(PointSeries::of_element(
                ...
                return Err(Error::new(ErrorKind::Other, format!("Reference point error: {:?}", e)));
            }
            ...
            if let Err(e) = chart.draw_series(PointSeries::of_element(
                ...
                return Err(Error::new(ErrorKind::Other, format!("Reference label error: {:?}", e)));
            }
        }
        ...
        if let Err(e) = chart.draw_series(LineSeries::new(
            vec![(0.0, 0.0, 0.0), (x_norm * 0.98, y_norm * 0.98, z_norm * 0.98)],
            RED.stroke_width(3),
        )) {
            return Err(Error::new(ErrorKind::Other, format!("Vector drawing error: {:?}", e)));
        }
        ...
        if let Err(e) = chart.draw_series(PointSeries::of_element(
            ...
            return Err(Error::new(ErrorKind::Other, format!("Point drawing error: {:?}", e)));
        }
        ...
        if let Err(e) = root.draw_text(
            ...
            return Err(Error::new(ErrorKind::Other, format!("Coordinates text error: {:?}", e)));
        }

        // Add state info
        let alpha = self.vector[0];
        let beta = self.vector[1];
        let state_text = format!("|ψ⟩ = {:.3}{:+.3}i|0⟩ + {:.3}{:+.3}i|1⟩",
        ...
        if let Err(e) = root.draw_text(
            ...
            return Err(Error::new(ErrorKind::Other, format!("State text error: {:?}", e)));
        }

        if let Err(e) = root.present() {
            return Err(Error::new(ErrorKind::Other, format!("SVG rendering error: {:?}", e)));
        }
