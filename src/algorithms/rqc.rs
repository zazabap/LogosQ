...
pub fn generate(config: &RandomCircuitConfig) -> Circuit {
    let mut circuit = Circuit::new(config.num_qubits)
        .with_name(&format!("Random Circuit (d={})", config.depth));
...
fn add_random_rotation_gate<R: Rng>(circuit: &mut Circuit, num_qubits: usize, rng: &mut R) {
    ...
    match rng.gen_range(0..3) {
        0 => { circuit.rx(qubit, angle); },
        1 => { circuit.ry(qubit, angle); },
        2 => { circuit.rz(qubit, angle); },
        _ => unreachable!(),
    }
}
...
pub fn generate_2design(num_qubits: usize) -> Circuit {
    ...
    let circuit = generate(&config)
        .with_name(&format!("2-Design Random Circuit (n={}, d={})", num_qubits, depth));
    circuit
}
...
pub fn generate_supremacy_circuit(num_qubits: usize, depth: usize) -> Circuit {
    let mut circuit = Circuit::new(num_qubits)
        .with_name(&format!("Supremacy Circuit (d={})", depth));
