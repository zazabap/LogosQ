...
mod tests {
    ...
    fn test_toffoli_distant_qubits() {
        ...
        assert!(state.probability(expected_state) > 0.99,
                ...
                (0..32).map(|i| format!("{}: {:.4}", i, state.probability(i)))
                       .collect::<Vec<_>>().join(", "));
        
        // Verify all other states have negligible probability
        for i in 0..32 {  // 2^5 = 32 possible states
            if i != expected_state {
                assert!(
    ...
    fn test_toffoli_quantum_addition() {
        ...
        assert!(state.probability(expected_state) > 0.99,
                ...
                (0..16).map(|i| format!("{}: {:.4}", i, state.probability(i)))
                       .collect::<Vec<_>>().join(", "));
    }
    ...
    fn test_toffoli_as_universal_gate() {
        ...
        assert!(cnot_state.probability(7) > 0.99,
                "Expected state 7 (111), got probabilities: {}",
                (0..8).map(|i| format!("{}: {:.4}", i, cnot_state.probability(i)))
                      .collect::<Vec<_>>().join(", "));
    }
    ...
    fn test_toffoli_with_entanglement() {
        ...
        assert!((state.probability(2) - 0.5).abs() < 1e-10,   // |0010⟩ = decimal 2
                "State 2 probability: {}", state.probability(2));
        assert!((state.probability(15) - 0.5).abs() < 1e-10,  // |1111⟩ = decimal 15
                "State 15 probability: {}", state.probability(15));
    ...
    fn test_toffoli_incremental_control() {
        ...
        assert!(state.probability(3) > 0.99,  // |011⟩ = decimal 3
                "Expected state 3 (011), got probabilities: {}",
                (0..8).map(|i| format!("{}: {:.4}", i, state.probability(i)))
                      .collect::<Vec<_>>().join(", "));
    }
